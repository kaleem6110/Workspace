...
<%		
		...
		instanceId = instanceId.substring(0,4).toLowerCase();
		...
		if( slidesBuilder!=null ) slidesBuilder= slidesBuilder.replaceAll("<img","<img alt=\"img\"");
		System.out.println(" slidesBuilder "+ slidesBuilder );
%>
 
<%if(displaySlide){ %>
	<div class="slider-wrapper theme-<%=themeValue%> <%=addCssClassValue%>">
	      <div id="<%=instanceId%>" class="nivoSlider">
	      	<%=slidesBuilder%>
		  </div>
	</div>
<%}else{%>
...
<script type="text/javascript" src="<%=renderRequest.getContextPath()%>/js/jquery.nivo.slider.js"></script>
<script type="text/javascript">
	$(window).load(function() {		
	    $("#<%=instanceId%>").nivoSlider({<%=buildSettings%>});
	});
</script>
