...
public class LDAPUtil 
{
	...
	public static DirContext  getLDAPContext(User user)
	{
		...
		String pwd = user.getOpenId();
		
		System.out.println("##### Mypwd : openid : "+pwd );
	...
	public static Attributes getAllAttributes(DirContext ctx, User user)
	{
		...
		NamingEnumeration answer = ctx.search(
				"ldap://ldap-dev.globalepic.lu:389/uid=" + user.getScreenName()
	...
	{	
		...
		try
		{
			...
			for (int j = 0; commuriList!=null&& j < commuriList.size(); j++)
			{
				...
				}else if (temp.indexOf("facebook") != -1) {
					ldapUserInfo.facebook = temp.replace("http:","");					
				}else if (temp.indexOf("twitter") != -1) {
					ldapUserInfo.twitter = temp.replace("http:","");					
				}else if (temp.indexOf("linkedin") != -1) {
					ldapUserInfo.mySpace = temp.replace("http:","");					
				}
	...
	public static void updatePassword(User user)
	{
		...
		{	
			...
			// System.out.println(" ############## 44444444 password :"+ user.getPassword() );
			 ...
			 System.out.println(" 1  setOpenId "+user.getPasswordUnencrypted() );
			...
			// Kaleem boolean isPwdModified = LiferayUsersMapDAO.storePassword( user.getUserId(),user.getPasswordUnencrypted() );
	...
	public static void updateContact(Contact  contact, boolean isBefore)
	{
		...
		{	
			 ...
			 String fb = contact.getFacebookSn();
			 String linkdin = contact.getMySpaceSn();
			 ...
			 if(fb!=null&& fb!="" )  mod0.add("http:"+fb );
			 if(linkdin!=null&& linkdin!="" )  mod0.add("http:"+linkdin );
			 if(twitter!=null&& twitter!="" )  mod0.add("http:"+twitter );
